<?php
/**
 * COmanage Registry Kerberos Authenticator Controller
 *
...
class KrbAuthenticatorsController extends SAuthController {
  // Class name, used by Cake
  public $name = "KrbAuthenticators";
  ...
  function beforeRender() {
    ...
    $this->set('vv_message_templates',
               $this->KrbAuthenticator->CoMessageTemplate->find('list', $args));
    
    if(!empty($this->viewVars['krb_authenticators'])) {
      // Construct the SSR initiation URL
      $url = array(
        'plugin'          => 'krb_authenticator',
        'controller'      => 'krbs',
        'action'          => 'ssr',
        'authenticatorid' => $this->viewVars['krb_authenticators'][0]['KrbAuthenticator']['authenticator_id']
      );
    ...
    if(!$this->request->is('restful')) {
      $this->set('vv_identifiers_types', $this->KrbAuthenticator->Authenticator->Co->CoPerson->Identifier->types($this->cur_co['Co']['id'], 'type'));
    }
  }
